...
| 5  | `rule_short_garbage`                                                      | *(skipped entirely if the line is `forgiven`, if `is_structured_line()` holds, or if `is_domain_notation()` holds — see `rule_domain_notation` below)* — `word_count ≤ ISOLATED_CHAR_MIN_TOKENS` (def: 3) **and** no Czech 🇨🇿 diacritics **and** (`lang_score ≤ LANG_SCORE_REMAP` (def: 0.75) **or** `rot_ratio ≥ SUSPICIOUS_ROT_RATIO` (def: 0.65)) **and** (gibberish present **or** `word_weird > 0`)                                                                                                                                                         | `Trash`                                                                           | Structural short-garbage route (e.g. `olie`). Recorded as `trash_threshold`. It does **not** consult `check_rescues()`. Since `070620f` (issue #30) it no longer returns unconditionally: it convicts only when `quality_score < CATEG_TRASH_SCORE_MAX + 0.35` **and** a second witness holds. On the short diacritic-free population `_has_strong_garbage_evidence()` returns `False` for *everything* — garbage and real domain vocabulary alike — so requiring it here was a suspension of the rule rather than a narrowing, at a measured cost of roughly **26,000 garbage lines reaching `Clear`**. `_has_shape_garbage_evidence()` (`SHORT_GARBAGE_WITNESS_ENABLE`, **off by default**) is the second witness that does discriminate; it is now wired here as an additional disjunct and is documented under §5c below. This is the **only** site in the short-line path that returns `Trash` — section 7's `damage` branch returns `Noisy` — which is why the witness belongs here and nowhere else. |
