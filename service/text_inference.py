...
Classification is fully aligned with the main pipeline (langID_classify.py):
  - Structural path  : categorize_line()   from text_util_langID
  - Ratio path       : classify_pipeline() from text_util_langID
  - Tiebreak         : compute_quality_score() / classify_by_score()
  - New API fields   : word_weird, quality_score
"""
...
# All classification logic lives there; the service must not duplicate it.
try:
    from text_util_langID import (
        # Line-level structural detectors
        ...
        # Classifiers
        categorize_line as _categorize_line_struct,   # perplexity + structural
        classify_pipeline,                             # ratio-based, no GPU
        classify_by_score,                             # score → label
        # Pre-filter (Empty / Non-text, CPU-only)
        pre_filter_line,
        # Perplexity
...
def _classify_line(
    ...
    *,
    ...
    """
    Run the full hybrid classification pipeline on a single text line and
    return all quality metrics.

    This mirrors the per-line logic inside process_and_write_batch() in
    langID_classify.py so that the service produces identical results.

    Returns a dict with keys:
        text, lang, lang_score, perplexity,
    ...
    # Hybrid classification — identical to langID_classify.py
    struct_cat = _categorize_line_struct(ppl, text, sym_count, upper_count)
    pipe_cat   = classify_pipeline(text)
...
def _classify_line_legacy(text: str, ppl: float, ft_model) -> Dict[str, Any]:
    ...
    Uses the old lang-score heuristic and reports word_weird / quality_score
    as None so the API caller can distinguish missing from zero.
    """
    labels, scores = ft_model.predict([text.lower()], k=1)
    lang       = labels[0][0].replace("__label__", "")
    lang_score = float(scores[0][0])
    categ      = _legacy_categorize(lang, lang_score, ppl, text)  # type: ignore[name-defined]
    return {
        "text":          text,
        "lang":          lang,
        "lang_score":    round(lang_score, 4),
