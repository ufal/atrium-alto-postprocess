...
def process_single_image(image_path: Path, output_path: Path, quality: int = 90) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        with Image.open(image_path) as img:
            metadata.update({"original_format": img.format, "mode_before": img.mode, "dimensions": img.size})
...
def img2jpeg_recursive(
    source_folder: str, backup_folder: str, output_folder: str, quality: int = 90
    ...
    for subdir in [source_path] + [p for p in source_path.rglob("*") if p.is_dir()]:
        ...
        for image_path in image_files:
            ...
            print(f"At {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: processed {len(metadata_list)} images total")
    ...
    if successful > 0:
        ...
        for m in metadata_list:
            if m.get("success", False):
                ...
                report.append(
                    f"- {orig_path.name} ({m['original_size'] / 1024:.1f} KB) to "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r", "--report", type=str, default=default_report, help=f"Report file path (default: {default_report})"
    )
    parser.add_argument(
        "-q", "--quality", type=int, default=default_quality, help=f"JPEG quality (1-100, default: {default_quality})"
    )
